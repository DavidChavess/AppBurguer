...
public class PromocaoMuitoQueijo implements PromocaoInterface{
	...
	public String getDescricao() {
		return "A cada 3 porções de queijo o cliente só paga 2, a cada 6 porções, o cliente pagará 4 e assim sucessivamente";
	}

	@Override
	public boolean podeReceberDesconto(Lanche lanche) {
		for(ItemLanche item : lanche.getIngredientes()) {
			if("Queijo".equalsIgnoreCase(item.getIngrediente().getNome())) {
